...
public partial class SettingsPage : ContentPage
{
	...
	protected override void OnAppearing()
	{
		...
		timerCell.Text = Properties.Resources.settings_timer_title + " " + PxUser.AppTimeout + " " +
						 Properties.Resources.settings_timer_unit_seconds;
	...
	private async void OnTimerTappedAsync(object sender, EventArgs e)
	{
		...
		var timerValue = await DisplayActionSheet(Properties.Resources.settings_timer_title,
							 Properties.Resources.action_id_cancel, null, timerlist.ToArray());
		if (timerValue == timer_30seconds)
			PxUser.AppTimeout = 30;
		else if (timerValue == timer_2minutes)
			PxUser.AppTimeout = 120;
		else if (timerValue == timer_5minutes)
			PxUser.AppTimeout = 300;
		else if (timerValue == timer_10minutes)
			PxUser.AppTimeout = 600;
		else if (timerValue == timer_30minutes)
			PxUser.AppTimeout = 1800;
	...
	private async void OnSwitcherToggledAsync(object sender, ToggledEventArgs e)
	{
		...
		if (e.Value)
			try
			{
				var data = await _currentUser.GetSecurityAsync();
				if (data == null)
				{

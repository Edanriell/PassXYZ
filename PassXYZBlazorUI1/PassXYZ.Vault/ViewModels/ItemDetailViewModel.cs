...
public partial class ItemDetailViewModel : BaseViewModel
{
    ...
    public async void Update(Field field)
	{
		...
		if (!field.IsBinaries && _item != null)
			...
			await Shell.Current.DisplayAlert(Properties.Resources.label_id_attachment,
				Properties.Resources.message_id_edit_binary, Properties.Resources.alert_id_ok);
	}
	...
	private async Task AddField(object obj)
	{
		...
		await Shell.Current.Navigation.PushAsync(new FieldEditPage(async (k, v, isProtected) =>
		{
			var field = _item.AddField(k, v, isProtected);
	...
	private async void AddBinaryField(string tempFilePath, string fileName)
	{
		...
		var field = _item.AddBinaryField(fileName, vBytes, Properties.Resources.label_id_attachment);
		if (field != null)
		{
			Fields.Add(field);
			await dataStore.UpdateItemAsync(_item);
